...
import '../view_model/employee_state.dart';
class EditEmployee extends StatefulWidget {
   EditEmployee({required this.employeeModel,super.key});
...
class _EditEmployeeState extends State<EditEmployee> {
  ...
  @override
  void initState() {
    name.text=widget.employeeModel.name;
    role.text=widget.employeeModel.role;
    joiningDate.text=widget.employeeModel.joiningDate.formatDate(shortName: true);
    if(widget.employeeModel.leavingDate!=null){
      leavingDate.text=widget.employeeModel.leavingDate!.formatDate(shortName: true);
    }
    joiningDateTime=widget.employeeModel.joiningDate;
    leavingDateTime=widget.employeeModel.leavingDate;
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return  BlocBuilder<EmployeeBloc, EmployeeState>(
      builder: (context, state)=> Scaffold(
        appBar: AppBar(
          title:  Text('Edit Employee Details',style: context.resources.textStyle.heading1,),
          automaticallyImplyLeading: false,
          actions: [
            IconButton(onPressed: (){
              context.read<EmployeeBloc>().add(DeleteSpecificData(employee: widget.employeeModel, context: context));
              context.pop();
            }, icon: Image.asset(
              Assets.iconsDelete,
              height: 18,
              ...
              children: [
                ...
                customButton(
                  ...
                  onTap: (){
                    if (name.text.isEmpty ||
                    ...
                    }else if (leavingDateTime!=null && leavingDateTime!.isBefore(joiningDateTime!)) {
                      AlertService.show(
                      ...
                      context.read<EmployeeBloc>().add(UpdateSpecificData(employee: widget.employeeModel, name: name.text, role: role.text, joiningDate: joiningDateTime ?? DateTime.now(),leavingDate: leavingDateTime));
                      context.pop();
                    }
                  },
                  text: 'Save',
